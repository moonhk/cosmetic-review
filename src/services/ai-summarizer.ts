...
import { createGroqChatCompletion } from "@/api/groq";

/**
 * AI 리뷰 요약 서비스
 * 비즈니스 로직을 담당 (프롬프트 생성, 응답 파싱)
 */
...
async function summarizeWithGroq(
  ...
  try {
    ...
    // 에러 메시지가 특정 형식이면 그대로 반환
    if (error instanceof Error && error.message.includes("AI 요청 한도")) {
 ...
 * 요약 프롬프트 생성
 */
function buildSummarizationPrompt(input: VideoSummaryInput): string {
  return `제품: ${input.brandName} ${input.productName}
리뷰 제목: ${input.title}
